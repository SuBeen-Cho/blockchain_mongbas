...
function strictPolicy(policy, logTrust) {
  if (!policy || typeof policy !== 'object' || Array.isArray(policy) ||
      Object.keys(policy).sort().join('\0') !== 'quorum\0schema\0witnesses' ||
      policy.schema !== 'mongbas-c2sp-witness-policy/v1' || !Array.isArray(policy.witnesses)) {
    throw new Error('unsupported C2SP witness policy');
  }
...
async function submit(requestFile, signedCheckpointFile, endpoint, logTrustFile, policyFile, outputFile) {
  const resolvedOutput = path.resolve(outputFile);
  if (fs.existsSync(resolvedOutput)) throw new Error('cosigned checkpoint output already exists');
  const request = readBoundedRegularFile(requestFile, 'C2SP witness request', 128 * 1024, { encoding: 'utf8' });
  const signedCheckpoint = readBoundedRegularFile(signedCheckpointFile, 'signed C2SP checkpoint', MAX_NOTE_BYTES, { encoding: 'utf8' });
  const logTrust = readJson(logTrustFile, 'C2SP log trust');
  const policy = strictPolicy(readJson(policyFile, 'C2SP witness policy'), logTrust);
  const result = await submitWitnessRequest({ endpoint, request, signedCheckpoint, logTrust, witnessPolicy: policy });
  exclusiveWrite(resolvedOutput, result.cosignedCheckpoint);
