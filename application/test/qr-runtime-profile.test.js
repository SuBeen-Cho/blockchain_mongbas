...
test('tailnet QR profile updater backs up and atomically restricts the backend', t => {
  ...
  assert.match(updated, /^CORS_ORIGIN=http:\/\/localhost:3000,https:\/\/node\.tail\.example$/m);
  const auditKey = updated.match(/^AUDIT_HMAC_KEY=(.+)$/m)?.[1];
  assert.ok(auditKey && Buffer.byteLength(auditKey) >= 32);
  assert.notEqual(auditKey, 'b'.repeat(32));
  assert.match(updated, new RegExp(`^DEMO_ADMISSION_FILE=${admissionPath.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')}$`, 'm'));
  assert.doesNotMatch(result.stdout + result.stderr, /a{32}|b{32}/);
