...
첫 `up`은 추적되지 않는 `network/.env`에 CouchDB 및 CA bootstrap 값을 난수로 생성하고 mode `0600`으로 보호합니다. 파일이 이미 있으면 덮어쓰지 않습니다. 이 파일을 Git에 추가하거나 다른 호스트와 공유하지 마세요. 기존 원장이 있는 환경에서는 `.env`를 재생성하거나 인증값을 임의로 바꾸지 말고 보존·회전 절차를 먼저 수립해야 합니다.
