...
for command_name in git node npm python3 openssl sha256sum; do
...
if [ -z "${branch}" ]; then
  printf 'ERROR: detached HEAD is not accepted for evidence runs\n' >&2
  exit 2
fi
...
if [ -n "$(git -C "${repo_dir}" status --porcelain=v1)" ]; then
  printf 'ERROR: verifier regression evidence requires a clean worktree\n' >&2
  exit 2
fi

install -d -m 0700 "${result_root}"
run_id="$(date -u +%Y%m%dT%H%M%SZ)"
final_dir="${result_root}/verifier-regression-${run_id}"
[ ! -e "${final_dir}" ] || { printf 'ERROR: result already exists: %s\n' "${final_dir}" >&2; exit 2; }
stage_dir="$(mktemp -d "${result_root}/.verifier-regression-${run_id}.tmp.XXXXXX")"
...
python3 --version >"${stage_dir}/python-version.txt" 2>&1
openssl version >"${stage_dir}/openssl-version.txt"
df -B1 "${repo_dir}" >"${stage_dir}/filesystem-before.txt"
free -b >"${stage_dir}/memory-before.txt" 2>&1 || true
uptime >"${stage_dir}/uptime-before.txt"
...
uptime >"${stage_dir}/uptime-after.txt"
printf '{"schema":"mongbas-verifier-regression-evidence/v1","commit":"%s","branch":"%s","testExitStatus":%s}\n' \
...
published=true
stage_dir=""
printf 'verifier regression evidence: %s\n' "${final_dir}"
exit "${test_status}"
