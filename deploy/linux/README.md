...
`coercion-evaluation.sh`는 격리 backend에서 normal/panic 증명 요청을 무작위 균형 순서로 수집한다. 응답 의미, byte size, latency를 raw JSONL로 보존하고, 훈련/평가를 분리한 threshold classifier와 Wilson 95% CI를 보고한다. 어떤 직접 구분자가 있으면 `securityGatePass=false`와 non-zero로 실패한다.
